...
public class Database {
    ...
    public static void init() {
        ...
        // ✅ Correct JDBC URL with default timezone (no TimeZone param)
        String jdbcUrl = "jdbc:postgresql://localhost:5432/ecosort?currentSchema=public";

        config.setJdbcUrl(jdbcUrl);
        config.setUsername("admin");      // DB username
        config.setPassword("admin123");   // DB password
    ...
    public static void createUser(String name, String email, String passwordHash) throws SQLException {
    ...
    public static void saveResult(int userId, String wasteType, String compositionJson) throws SQLException {
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "INSERT INTO results (user_id, waste_type, composition) VALUES (?, ?, ?)")) {
    ...
    public static String getHistoryJson(int userId) throws SQLException {
        ...
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT waste_type, composition, created_at FROM results WHERE user_id = ? ORDER BY created_at DESC")) {
            stmt.setInt(1, userId);
